...
void main() {
  ...
  group(
    'friendsUnlocked (this task\'s requirement — the tab stays inactive '
    ...
    () {
      ...
      test(
        'locked once logged in but the profile/nickname has not resolved '
        'yet',
        () {
          when(
      ...
      test('unlocked once logged in and the profile has a nickname', () async {
        when(() => userProfileRepository.watchProfile('me')).thenAnswer(
          (_) => Stream.value(
            const FriendProfile(uid: 'me', nickname: 'pupa', avatarPresetIndex: 0),
          ),
        );
        final container = buildContainer(
          authState: const AuthState(uid: 'me', isAnonymous: false),
