...
void main() {
  ...
  test('sendRequest creates a pending document keyed by the sorted pair id', () async {
    await repository.sendRequest(fromUid: 'bob', toUid: 'alice');

    final doc = await firestore.collection('friendships').doc('alice_bob').get();
  ...
  test('removeOrDecline deletes the document, allowing a fresh request after', () async {
    final pairId = pairIdFor('alice', 'bob');
    await repository.sendRequest(fromUid: 'bob', toUid: 'alice');

    await repository.removeOrDecline(pairId);
    final deleted = await firestore.collection('friendships').doc(pairId).get();
    ...
    final recreated = await firestore.collection('friendships').doc(pairId).get();
  ...
  test('watchMyFriendships returns every friendship uids contains this uid', () async {
    await repository.sendRequest(fromUid: 'alice', toUid: 'bob');
    await repository.sendRequest(fromUid: 'alice', toUid: 'carol');
    await repository.sendRequest(fromUid: 'dave', toUid: 'erin'); // unrelated pair
  ...
  test('a friendship round-trips through Firestore with matching fields', () async {
    await repository.sendRequest(fromUid: 'alice', toUid: 'bob');
    await repository.acceptRequest(pairIdFor('alice', 'bob'));

    final friendship = (await repository.watchMyFriendships('alice').first).single;
