...
void main() {
  ...
  group(
    'an already-linked Google identity — "repeat login" switches to the '
    ...
    () {
      ...
      test(
        'switches the session — existingAccountRestored, uid updated, '
        'no longer anonymous',
        () async {
          when(
      ...
      test(
        'the local total is bigger — drift is left alone, cloud is pushed '
        'up to match it',
        () async {
          when(
            () => progressSyncRepository.fetchCurrentProgress('existing-uid'),
      ...
      test(
        'a reconciliation failure (e.g. offline) never undoes the '
        'already-succeeded account switch',
        () async {
          when(
            () => progressSyncRepository.fetchCurrentProgress('existing-uid'),
